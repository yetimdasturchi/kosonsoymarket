...
class Database {
  ...
  $raw_positions_row =  unpack('V', substr($this->raw_positions_row, $pos, 4));
  $spos = $raw_positions_row[1] + $additional;
  ...
  private function readFloat($pos) {
    ...
    $raw_positions_row = unpack('f', substr($this->raw_positions_row, $pos, self::$floatSize));
    return $raw_positions_row[1];
  }
  ...
  private function readWord($pos) {
    ...
    $raw_positions_row = unpack('V', $this->read($pos - 1, 4));
    return self::wrap32($raw_positions_row[1]);
  }
  ...
  private function readByte($pos) {
    ...
    $raw_positions_row = unpack('C', $this->read($pos - 1, 1));
    return self::wrap8($raw_positions_row[1]);
  }
      ...
      switch ($afield) {
        ...
        case self::COUNTRY_CODE:
          if (!$done[self::COUNTRY_CODE]) {
            $readCountryNameAndCode = $this->readCountryNameAndCode($pointer);
            $results[self::COUNTRY_CODE] = $readCountryNameAndCode[1];
            $done[self::COUNTRY_CODE]    = true;
          }
          break;
        case self::COUNTRY_NAME:
          if (!$done[self::COUNTRY_NAME]) {
            $readCountryNameAndCode = $this->readCountryNameAndCode($pointer);
            $results[self::COUNTRY_NAME] = $readCountryNameAndCode[0];
            $done[self::COUNTRY_NAME]    = true;
          }
          break;
        ...
        case self::LATITUDE:
          if (!$done[self::LATITUDE]) {
            $readLatitudeAndLongitude = $this->readLatitudeAndLongitude($pointer);
            $results[self::LATITUDE] = $readLatitudeAndLongitude[0];
            $done[self::LATITUDE]    = true;
          }
          break;
        case self::LONGITUDE:
          if (!$done[self::LONGITUDE]) {
            $readLatitudeAndLongitude = $this->readLatitudeAndLongitude($pointer);
            $results[self::LONGITUDE] = $readLatitudeAndLongitude[1];
            $done[self::LONGITUDE]    = true;
          }
          break;
        case self::ISP:
        ...
        case self::IDD_CODE:
          if (!$done[self::IDD_CODE]) {
            $readIddAndAreaCodes = $this->readIddAndAreaCodes($pointer);
            $results[self::IDD_CODE] = $readIddAndAreaCodes[0];
            $done[self::IDD_CODE]    = true;
          }
          break;
        case self::AREA_CODE:
          if (!$done[self::AREA_CODE]) {
            $readIddAndAreaCodes = $this->readIddAndAreaCodes($pointer);
            $results[self::AREA_CODE] = $readIddAndAreaCodes[1];
            $done[self::AREA_CODE]    = true;
          }
          break;
        case self::WEATHER_STATION_CODE:
          if (!$done[self::WEATHER_STATION_CODE]) {
            $readWeatherStationNameAndCode = $this->readWeatherStationNameAndCode($pointer);
            $results[self::WEATHER_STATION_CODE] = $readWeatherStationNameAndCode[1];
            $done[self::WEATHER_STATION_CODE]    = true;
          }
          break;
        case self::WEATHER_STATION_NAME:
          if (!$done[self::WEATHER_STATION_NAME]) {
            $readWeatherStationNameAndCode = $this->readWeatherStationNameAndCode($pointer);
            $results[self::WEATHER_STATION_NAME] = $readWeatherStationNameAndCode[0];
            $done[self::WEATHER_STATION_NAME]    = true;
          }
          break;
        case self::MCC:
          if (!$done[self::MCC]) {
            $readMccMncAndMobileCarrierName = $this->readMccMncAndMobileCarrierName($pointer);
            $results[self::MCC] = $readMccMncAndMobileCarrierName[0];
            $done[self::MCC]    = true;
          }
          break;
        case self::MNC:
          if (!$done[self::MNC]) {
            $readMccMncAndMobileCarrierName = $this->readMccMncAndMobileCarrierName($pointer);
            $results[self::MNC] = $readMccMncAndMobileCarrierName[1];
            $done[self::MNC]    = true;
          }
          break;
        case self::MOBILE_CARRIER_NAME:
          if (!$done[self::MOBILE_CARRIER_NAME]) {
            $readMccMncAndMobileCarrierName = $this->readMccMncAndMobileCarrierName($pointer);
            $results[self::MOBILE_CARRIER_NAME] = $readMccMncAndMobileCarrierName[2];
            $done[self::MOBILE_CARRIER_NAME]    = true;
          }
          break;
        case self::ELEVATION:
